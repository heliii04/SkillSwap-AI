...
export default function Notifications() {
    ...
    const handleNotificationClick = async (notification) => {
        ...
        if (notification.link) {
            ...
            navigate("/messages");
        } else if (notification.type === "ai_suggestion") {
    ...
    const filteredNotifications = useMemo(() => {
        if (activeTab === "requests") {
            return notifications.filter((n) =>
                ["swap_request", "swap_accepted", "swap_rejected"].includes(
                    n.type
                )
            );
        }
    ...
    const counts = useMemo(() => {
        return {
            ...
            suggestions: notifications.filter(
                (n) => !n.isRead && n.type === "ai_suggestion"
            ).length,
        };
    }, [notifications]);
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {notifications.some(n => !n.isRead) && (
                        <button
                            type="button"
                            onClick={markAllRead}
                            className="inline-flex items-center gap-2 self-start rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm  text-white transition hover:bg-white/[0.08] font-bold"
                        >
                        ...
                        {filteredNotifications.map(notification => (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className="opacity-0 group-hover:opacity-100 self-start rounded-lg p-1.5 text-white/20 transition hover:bg-white/5 hover:text-white font-bold"
                                >
